...
(function () {
  ...
  var INLINE_SRC = [
    ...
    '\\*([^*\\n]+)\\*',                        // 9    italic
    '(https?://[^\\s<>"\')]+)',                // 10   bare url
    '(<\\s*/?\\s*br\\s*/?\\s*>)'               // 11   <br>, however spelt
  ].join('|');
  ...
  function mdInline(parent, text, inLink) {
    ...
    while ((m = re.exec(text)) !== null) {
      ...
      } else if (m[11] !== undefined) {               // <br>, <br/>, </br> ...
        /*
         * NIP-23 content is markdown, and this renderer never interprets html
         * -- but authors and cross-posting tools leave line-break tags in the
         * text all the same, and printing "<br />" as prose is worse than
         * honouring the break. Only this one tag is understood; anything else
         * html-shaped still comes out as literal text, which is the point.
         */
  ...
  function bookmarkCard(ev, pubkey) {
    ...
    if (footer.firstChild) main.appendChild(footer);
  ...
  function highlightCard(ev) {
    ...
    // Nothing quoted, nothing said, nowhere to point: no card to draw.
    if (!passage.mark && !comment && !source) return null;
    ...
    if (footer.firstChild) item.appendChild(footer);
  ...
  function renderBookmarks(events, ctx) {
    ...
    items.forEach(function (ev) {
      var card = ev.kind === BOOKMARK_KIND ? bookmarkCard(ev, ctx.pubkey) : highlightCard(ev);
      if (card) { frag.appendChild(card); count++; }
    });
    return { node: frag, count: count };
  }
